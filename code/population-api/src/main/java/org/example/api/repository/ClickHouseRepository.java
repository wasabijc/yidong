...
public class ClickHouseRepository {
    ...
    public long queryTotal(String regionId, LocalDateTime startTime, LocalDateTime endTime) {
        String sql = "SELECT COUNT(DISTINCT imsi) AS total FROM population_data " +
                "WHERE region_id = ? AND event_time BETWEEN ? AND ?";
    ...
    public Map<String, Long> queryAgeDistribution(String regionId, LocalDateTime startTime, LocalDateTime endTime) {
        String sql = "SELECT age_group, COUNT(DISTINCT imsi) AS count FROM population_data " +
                "WHERE region_id = ? AND event_time BETWEEN ? AND ? " +
                "GROUP BY age_group";
        return executeGroupQuery(sql, regionId, startTime, endTime);
    }
    ...
    public Map<String, Long> queryGenderDistribution(String regionId, LocalDateTime startTime, LocalDateTime endTime) {
        String sql = "SELECT gender, COUNT(DISTINCT imsi) AS count FROM population_data " +
                "WHERE region_id = ? AND event_time BETWEEN ? AND ? " +
                "GROUP BY gender";
        return executeGroupQuery(sql, regionId, startTime, endTime);
    }
